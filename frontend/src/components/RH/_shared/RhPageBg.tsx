...
// Background reutilizável das páginas RH/DP.
// Tema padrão: rose/pink. Tema 'navy' p/ Documentos/Equipamentos/Configurações.
const RhPageBg: React.FC<{ tema?: 'rose' | 'navy'; children: React.ReactNode }> = ({ tema = 'rose', children }) => {
    if (tema === 'navy') {
        return (
            <div className="-m-4 md:-m-6 lg:-m-8 min-h-[calc(100vh-2rem)] relative overflow-hidden bg-gradient-to-br from-slate-50 via-blue-50/60 to-indigo-50 dark:from-slate-900 dark:via-blue-950/40 dark:to-indigo-950/40">
                ...
                <div className="pointer-events-none absolute inset-0 opacity-[0.04] dark:opacity-[0.08]"
                    style={{ backgroundImage: 'linear-gradient(rgb(30,58,138) 1px, transparent 1px), linear-gradient(90deg, rgb(30,58,138) 1px, transparent 1px)', backgroundSize: '40px 40px' }} />
                <div className="relative p-4 sm:p-6 max-w-[1400px] mx-auto space-y-4">{children}</div>
            </div>
        );
    }
    // tema 'rose' (default)
    return (
        <div className="-m-4 md:-m-6 lg:-m-8 min-h-[calc(100vh-2rem)] relative overflow-hidden bg-gradient-to-br from-rose-50 via-pink-50/50 to-fuchsia-50 dark:from-slate-900 dark:via-rose-950/40 dark:to-fuchsia-950/40">
            ...
            <div className="pointer-events-none absolute inset-0 opacity-[0.04] dark:opacity-[0.08]"
                style={{ backgroundImage: 'linear-gradient(rgb(190,18,60) 1px, transparent 1px), linear-gradient(90deg, rgb(190,18,60) 1px, transparent 1px)', backgroundSize: '40px 40px' }} />
            <div className="relative p-4 sm:p-6 max-w-[1400px] mx-auto space-y-4">{children}</div>
        </div>
    );
};

export default RhPageBg;
