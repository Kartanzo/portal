...
import {
    Layers, Calendar, User as UserIcon, AlertCircle, CheckCircle2, Clock, Ban,
    X, Edit3, Calculator, PlayCircle, MessageSquare, Lock, Save, Target, FileText
} from 'lucide-react';
...
const StrategicKanban: React.FC<StrategicKanbanProps> = ({ user }) => {
    ...
    const accessibleUsers = useMemo(() => {
        ...
        return allUsers.filter(u => checkUserModuleAccess(u, 'strategic_kanban'));
    }, [allUsers, strategicData, userRole]);

    const perm_kanban = user.permissions?.['strategic_kanban'];
    const hasSpecificSectors_kanban = perm_kanban?.allowed_sectors && perm_kanban.allowed_sectors.length > 0;
    const canViewAllSectors = !hasSpecificSectors_kanban && (userRole === 'super_user' || userRole === 'ceo' || perm_kanban?.view_all_sectors);
    ...
    const sectorOptions = useMemo(() => {
        ...
        if (hasSpecificSectors_kanban) {
            const sectorMode = (perm_kanban as any)?.sector_mode || 'include';
            let roleSectors: string[];
            if (sectorMode === 'include') {
                roleSectors = [...perm_kanban!.allowed_sectors!];
            } else {
                roleSectors = SECTORS.filter(s => !perm_kanban!.allowed_sectors!.includes(s));
            }
        ...
        return ['Todos', ...base];
    }, [user, canViewAllSectors, hasSpecificSectors_kanban, SECTORS, strategicData, userRole]);
    ...
    useEffect(() => {
        if (!canViewAllSectors && !selectedSectorFilter) { }
    }, [user.sector, canViewAllSectors]);

    useEffect(() => {
        loadPlans();
    }, [user, canViewAllSectors, sectorOptions]);
    ...
    const loadPlans = async () => {
        try {
            setLoading(true);
            const data = await api.getActionPlans(undefined, user.id);

            // STRICTOR FILTER: Filter results by sectorOptions list
            const allowedSet = new Set(sectorOptions);
            let finalData = data;

            if (!canViewAllSectors || sectorOptions.length <= SECTORS.length) {
                finalData = data.filter((plan: ActionPlanItem) => {
                    const planSector = (plan.sector || '').trim();
                    return allowedSet.has('Todos') || allowedSet.has(planSector);
                });
            }
    ...
    const filterValidUsers = (list: string | string[], sector: string) => {
        ...
        return currentList.filter(name => {
            ...
            const user = accessibleUsers.find(u => u.name.toLowerCase() === name.toLowerCase());
            return !!user;
        });
    };
    ...
    const kanbanItems = useMemo(() => items.flatMap((theme, themeIdx) =>
        ...
        const matchesSector = !selectedSectorFilter || selectedSectorFilter === 'Todos' ||
            (item.sector && item.sector === selectedSectorFilter);

        // 3. Status Filter
        const matchesStatus = !selectedStatusFilter || selectedStatusFilter === 'Todos' || item.status === selectedStatusFilter;

        // 4. Created By Filter
        const matchesCreatedBy = !selectedCreatedByFilter || item.createdByName === selectedCreatedByFilter;

        return matchesSearch && matchesSector && matchesStatus && matchesCreatedBy;
    }), [items, searchQuery, selectedSectorFilter, selectedStatusFilter, selectedCreatedByFilter, accessibleUsers]);
    ...
    const onDrop = async (e: React.DragEvent, targetStatus: string) => {
        ...
        if (foundSub) {
            setItems(newItems);
            try {
                await api.updateActionPlanItem(itemId, {
                    ...
                    updatedBy: user.id // CRITICAL: backend needs this to save the change
                });
            } catch (err) {
        ...
        <div className="space-y-6 h-full flex flex-col">
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 shrink-0">
                <div className="flex items-center space-x-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-black text-slate-900 uppercase tracking-tighter">Kanban Estratégico</h1>
                        <p className="text-gray-500 text-sm font-medium italic">Visualização de ações por status</p>
                    </div>
                </div>
            </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div className="relative">
                        <select
                            ...
                            // disabled={!canViewAllSectors} // No longer disable, as they might want to switch between their managed sectors
                    ...
                    <div className="relative">
                        <select
                            ...
                            onChange={(e) => setSelectedCreatedByFilter(e.target.value)}
                            className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold uppercase text-slate-600 outline-none focus:ring-2 focus:ring-slate-900 appearance-none cursor-pointer"
                        >
                                            ...
                                            <div className="flex items-center justify-between pt-1.5 border-t border-gray-50 text-[8px] text-gray-400 font-bold uppercase">
                                                ...
                                                <div className="flex -space-x-1">
                                                    {Array.isArray(item.responsible) ?
                                                        item.responsible.slice(0, 2).map((r, i) => (
                                ...
                                <div className="grid grid-cols-2 gap-6 border-t border-slate-50 pt-4">
                                    <div>
                                        ...
                                        <div className="mt-1 flex items-center text-sm font-medium text-slate-700">
                                            ...
                                            {(() => {
                                                // Filter to show only accessible users
                                                ...
                                                const filteredResponsible = responsibleList.filter((r: string) => accessibleUsers.some(u => u.name === r));
                        ...
                        <div className="bg-white rounded-2xl shadow-2xl max-w-5xl w-full overflow-hidden flex flex-col animate-in slide-in-from-bottom-4 duration-300" onClick={e => e.stopPropagation()}>
                            <div className="p-6 bg-red-600 text-white flex justify-between items-center shrink-0">
                                <div>
                                    ...
                                    <p className="text-[9px] opacity-80 mt-1">Gestão de Planejamento - Setor: {canViewAllSectors && !selectedItem.sector ? 'Todos' : (selectedItem.sector || user.sector)}</p>
                                </div>
                            ...
                            <form className="p-8 space-y-6 overflow-y-auto max-h-[75vh]" onSubmit={async (e) => {
                                e.preventDefault();
                                try {
                                    await api.updateActionPlanItem(selectedItem.id, {
                                        ...formData,
                                        updatedBy: user.id
                                    });
                                            ...
                                            return (
                                                <>
                                                    <MultiSelectDropdown
                                                        ...
                                                        selected={(formData.responsible || []).filter((r: string) => accessibleUsers.some(u => u.name === r))}
                                                    ...
                                                    <MultiSelectDropdown
                                                        ...
                                                        options={accessibleUsers
                                                            .map(u => ({ id: u.id, name: u.name, sector: u.sector }))}
                                                        selected={(formData.waitingForReturn || []).filter((r: string) => accessibleUsers.some(u => u.name === r))}
...
export default StrategicKanban;
