...
const ImplementationDashboard: React.FC<ActionPlanDashboardProps> = ({ user }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        // If restricted, we might need to fetch ALL and filter currently, OR better:
        // ActionPlan logic: fetch(undefined) returns ALL. We filter client side for security if not super user.
        // If we passed a specific sector to API, we might miss the user's other managed sectors.
        // So we request ALL (undefined) and filter here.
  ...
  const stats = useMemo(() => {
    ...
    filteredData.forEach(plan => {
      ...
      if (!sectorMap[sectorName]) {
        sectorMap[sectorName] = { total: 0, concluded: 0, statuses: {}, categories: new Set() };
      }
      const sStats = sectorMap[sectorName];
      ...
      plan.subItems.forEach(action => {
        ...
        if (plan.objective) {
          sStats.categories.add(plan.objective);
        }
    ...
    // Calculate total unique categories across all sectors
    const totalCategories = Object.values(sectorMap).reduce((sum, sec: any) => sum + sec.categories.size, 0);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-6">
        <div>
          <h1 className="text-2xl font-black text-slate-900 uppercase tracking-tighter">Gestão de Projetos</h1>
          <p className="text-gray-500 text-sm font-medium italic">Visão consolidada da Gestão de Projetos e KPIs.</p>
        </div>
      ...
      <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col">
        <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest mb-6 flex items-center gap-2">
          <ClipboardList className="w-4 h-4 text-slate-400" /> Detalhamento por Setor
        </h3>
        <div className="space-y-4">
          {Object.keys(stats.sectorDetails).sort().map(sector => {
            const sStats = stats.sectorDetails[sector];
            return (
              <SectorItem
                key={sector}
                name={sector}
          ...
          {Object.values(stats.sectorDetails).every((m: any) => m.total === 0) && <p className="text-xs text-gray-400 italic">Nenhum dado disponível.</p>}
        </div>
      </div>
    </div>
  );
