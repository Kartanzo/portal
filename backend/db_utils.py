...
def get_db_connection():
    ...
    # 1) DATABASE_URL (ex.: EasyPanel) — psycopg2 entende a URL direto (inclui sslmode).
    database_url = os.environ.get("DATABASE_URL")
    if database_url:
        conn = psycopg2.connect(database_url, connect_timeout=10)
    else:
        # 2) Variáveis DB_* separadas (Docker/Produção)
        ...
        if db_host and db_name and db_user and db_pass:
            ...
            # 3) Fallback to local cred.json
            if os.path.exists('cred.json'):
                with open('cred.json', 'r') as f:
                    creds = json.load(f)
            else:
                raise Exception("No database credentials found (DATABASE_URL, Env Vars or cred.json)")
